...
exports.registerEventsRoutes = registerEventsRoutes;
const zod_1 = require("zod");
const severityEnum = zod_1.z.enum(['INFO', 'WARN', 'CRITICAL']);
function registerEventsRoutes(app) {
    app.get('/api/events', { preHandler: app.auth.requireApiKey('ADMIN') }, async (request) => {
        ...
        if (severityEnum.safeParse(severity).success)
    ...
    app.get('/api/events/summary', { preHandler: app.auth.requireApiKey('ADMIN') }, async () => {
        const now = new Date();
    ...
    app.get('/api/events/timeseries', { preHandler: app.auth.requireApiKey('ADMIN') }, async (request) => {
